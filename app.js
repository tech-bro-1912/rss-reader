...
// ══
// Palette de couleurs pour les flux
const FEED_COLORS = [
...
// Suffixes RSS à tester lors de la découverte
const RSS_SUFFIXES = [
  '/feed', '/feed/', '/feed.xml', '/feed.rss', '/feed.atom',
  '/rss', '/rss/', '/rss.xml', '/rss2.xml',
  '/atom', '/atom.xml',
...
// Flux de découverte pour l'explorateur
const DISCOVERY_FEEDS = [
  ...
  { name: 'Libération',            url: 'https://www.liberation.fr/arc/outboundfeeds/rss/',         icon: '📢' },
  ...
  { name: 'Alternatives Éco.',     url: 'https://www.alternatives-economiques.fr/rss.xml',          icon: '💡' },
  { name: 'The Conversation',      url: 'https://theconversation.com/fr/articles.atom',             icon: '🎓' },
  { name: 'Monde Diplomatique',    url: 'https://www.monde-diplomatique.fr/rss/',                   icon: '🗺️' },
  { name: 'Numerama',              url: 'https://www.numerama.com/feed/',                           icon: '💻' },
  { name: 'Next INpact',           url: 'https://www.nextinpact.com/rss/news.xml',                  icon: '🔧' },
];

// Variables globales de l'explorateur
...
function setSwipeHeight() {
  ...
  if (!refHeader) return;
  if (headerH < 10) return; // pas encore rendu
  ...
  document.querySelectorAll('.swipe-view').forEach(view => {
    ...
    Array.from(view.children).forEach(child => {
      const isScroll = child.classList.contains('scroll-area')
    ...
    const scrollH = containerH - fixedH;
    if (scrollH < 50) return;

    // Appliquer à tous les scroll-areas (même cachés)
    view.querySelectorAll('.scroll-area, .search-results-area').forEach(sa => {
      sa.style.height = scrollH + 'px';
      sa.style.flex = 'none';
      sa.style.overflowY = 'auto';
    });
    ['podcastList', 'episodeList'].forEach(id => {
...
// ── parseFeed : parse RSS 2.0, Atom 1.0 et JSON Feed ──
function parseFeed(text, url) {
  ...
  try {
    ...
    throw new Error('Impossible de parser le XML : ' + e.message);
  }
  ...
  if (isAtom) {
    ...
    const items = [...doc.querySelectorAll('entry')].map(e => {
      ...
      return {
        ...
        id:          e.querySelector('id')?.textContent || link,
      };
    });
  ...
  if (!channel) throw new Error('Format non reconnu (pas de <channel> ni <feed>)');
  ...
  const items = [...doc.querySelectorAll('item')].map(item => {
    ...
    const rawContent = item.querySelector('encoded')?.textContent
                    || item.querySelector('description')?.textContent || '';
    const imgMatch = rawContent.match(/<img[^>]+src=["']([^"']+)["']/i);
    const mediaThumb = item.querySelector('thumbnail')?.getAttribute('url')
                    || item.querySelector('content[medium="image"]')?.getAttribute('url') || '';
    const enclosure = item.querySelector('enclosure');
    const enclosureUrl = enclosure?.getAttribute('url') || '';
    const isImage = enclosure?.getAttribute('type')?.startsWith('image');
    ...
    return {
      ...
      image:       mediaThumb || (isImage ? enclosureUrl : '') || (imgMatch ? imgMatch[1] : ''),
...
async function importFromFile(event) { return importFeedFile(event); }
...
function showFluxList() {
  ...
  document.getElementById('feedSubHeader').style.display = 'none';
  const _sb1 = document.querySelector('.search-bar'); if (_sb1) _sb1.style.display = 'none';
  const _sa1 = document.getElementById('searchAdv'); if (_sa1) _sa1.classList.remove('open');
  // Header
...
function showArticleList() {
  ...
  document.getElementById('fluxPanel').classList.remove('active');
  document.getElementById('articleList').style.display = '';
  const _sb2 = document.querySelector('.search-bar'); if (_sb2) _sb2.style.display = '';
}
...
function openFeedFilter(feedId) {
  ...
  document.getElementById('fluxPanel').classList.remove('active');
  document.getElementById('articleList').style.display = '';
  const _sb3 = document.querySelector('.search-bar'); if (_sb3) _sb3.style.display = '';
  // Deselect all tabs
  document.querySelectorAll('.tab').forEach(t => t.classList.remove('active'));
  renderArticles();
}
...
async function addPodcastByUrl() { addPodcast(); }
...
function switchPodMode(tab) { setPodSrcTab(tab); }
...
// Alias pour toggleFluxSelector / togglePodSelector (noms utilisés dans le HTML)
...
function initSplash() {
  ...
  if (!splash) { console.error('[initSplash] splashScreen introuvable'); return; }
  ...
  setTimeout(() => {
    ...
    } catch(e) { console.error('[initSplash] refresh error:', e); }
  }, 500);

  // Disparaître après 3s — blindé quoi qu'il arrive
  function hideSplash() {
    try {
  ...
  // Filet de sécurité : forcer à 5s si tout le reste échoue
  setTimeout(() => {
    try { splash.style.display = 'none'; splash.style.zIndex = '-1'; } catch(_) {}
  }, 5000);
}

// ── Démarrage ──
// initSplash EN PREMIER pour garantir l'enregistrement des timers
try { initSplash(); } catch(e) { console.error('[INIT] initSplash() failed:', e); }

// Initialisation dans un setTimeout pour ne pas bloquer le thread
// avant que les timers du splash soient enregistrés
setTimeout(function() {
  try { load(); } catch(e) { console.error('[INIT] load() failed:', e); }
  try { showFluxList(); } catch(e) { console.error('[INIT] showFluxList() failed:', e); }
  try { updateFilterTabs(); } catch(e) { console.error('[INIT] updateFilterTabs() failed:', e); }
}, 0);
